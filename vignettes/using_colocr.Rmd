...
```{r call_parameter_show2, fig.width=7, fig.height=7}
# show parameters for the selected rois
parameter_show(img, px, labs.px)
```

```{r call_coloc_test3}
# apply colocalization stats on the specific rois
corr2 <- coloc_test(img, px, labs.px, type = 'all', num = TRUE)
...
px <- parameter_choose(ex1, 
                       threshold = 90, 
                       shrink = 8)

# add labels
labs.px <- labels_add(px, n = 50)

# show parameters
parameter_show(ex1, px, labs.px)

# test colocalization
pearsons <- coloc_test(ex1, px, labs.px)

# results
mean(pearsons$p, na.rm = TRUE)
```
...
# add labels
labs.px <- labels_add(px, n = 20)

# show parameters
parameter_show(ex3, px, labs.px)

# test colocalization
pearsons <- coloc_test(ex3, px, labs.px, type = 'all')
