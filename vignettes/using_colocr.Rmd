...
The `colocr` package provides a straight forward workflow for determining the amount of colocalization. This workflow consists of three steps only:

  * Choosing regions of interest (ROI)
  * Getting the pixel intensities from the two channels
  * Calculating the correlation between the pixel intensites
...
Although this app was designed to be invoked from within the package to help the users to choose the selection parameters interactively. It's a stand alone app and can run the same analysis descriped here. The app can be accessed from the web [here](https://mahshaaban.shinyapps.io/colocr_app2/)
...
corr2 <- coloc_test(pix_int2, type = 'all')
...
# test colocalization
pix_int01 <- intensity_get(ex1, labs.px)

corr <- coloc_test(pix_int01, type = 'all')
...
# test colocalization
pix_int03 <- intensity_get(ex3, labs.px)

corr <- coloc_test(pix_int03, type = 'all')
