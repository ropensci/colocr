...
This packge depends on `imager` which has some external dependencies. The instructions for
...
```{r apply_inputs}
# read images
imgs <- map(inputs$image, function(x) {
...
all(round(stats$pcc, 2) == round(c(rep_stats[[1]]$pcc, rep_stats[[2]]$pcc), 2))
all(round(stats$moc, 2) == round(c(rep_stats[[1]]$moc, rep_stats[[2]]$moc), 2))
```
